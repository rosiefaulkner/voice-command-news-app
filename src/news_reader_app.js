...
intent("Give me the news from $(source* (.*))", (p) => {
  ...
  api.request(
    ...
    (error, response, body) => {
      ...
      if (!articles.length) {
        p.play(
          `Sorry, I\'m having trouble finding news from ${p.source.value}. Please try searching for news from somewhere else.`
        );
        return;
      }
      ...
      p.play(
        `Here are the (latest|most recent) headlines from ${p.source.value}.`
      );
...
intent("What's up with $(term* (.*))", (p) => {
  ...
  api.request(
    ...
    (error, response, body) => {
      ...
      if (!articles.length) {
        p.play(
          `I can\'t find any news about ${p.term.value}. Look for news about something else.`
        );
        return;
      }
      ...
      p.play({ command: "newHeadlines", articles });
      p.play(
        `Here are the (latest|most recent) headlines about ${p.term.value}.`
      );
...
intent(
  ...
  `(read|show|get|bring me) (the|) (recent|latest|) $(C~ ${CATEGORIES_INTENT}) $(N news|headlines)`,
  (p) => {
    let NEWS_API_URL = `https://newsapi.org/v2/top-headlines`;
    ...
    api.request(
      ...
      (error, response, body) => {
        ...
        if (!articles.length) {
          p.play(
            `I can\'t find any news about ${p.C.value}. Look for a different category.`
          );
          return;
        }
        ...
        if (p.C.value) {
          p.play(
            `Here are the (latest|most recent) articles about ${p.C.value}.`
          );
        } else {
...
intent("how are you", reply("I'm doing well. (Thank you|)"));
...
function apiCall(p, command, param, callback) {
  ...
  api.request(jsp, (err, res, body) => {
    if (err || res.statusCode !== 200) {
      p.play(
        `(Sorry|) something went wrong (on the server|) ${err} ${res} ${body}`
      );
    } else if (body.error) {
      p.play(body.error);
    } else {
  ...
  (p) => {
    ...
    api.request(
      ...
      (error, response, body) => {
        ...
        if (!articles.length) {
          p.play(
            `I can\'t find any news about ${p.T.value}. Look for a different category.`
          );
          return;
        }
        ...
        if (p.T.value) {
          p.play(
            `Here are the (latest|most recent) articles about ${p.T.value}.`
          );
        } else {
          p.play(`Here are the (latest|most recent) headlines`);
        }
